...
SHARP_TURN_POINTS = list(range(21, 42))    

# Must update if the env changes.
TOP_SPEED = 4.0  
IS_CLOCKWISE = False
INTERVAL_TO_TARGET_WAY_POINT = 2
STEP_VALUE = 0.2
...
def reward_function(params):
	...
	# 3. Encourage the agent to have a high speed towards a far side way point (if not a sharp turning).
	if closest_waypoints[1] in (SHARP_TURN_POINTS):
		speed_bonus = 1
	else:
	...
	reward += speed_bonus * (distance_2_waypoint_bonus**2)
